const { uploadMusicService, createAlbumService, addMusicToAlbumService, removeMusicFromAlbumService, deleteAlbumService } = require('../services/artistService');
...
const uploadMusical = [upload.fields([{ name: 'musicFile', maxCount: 1 }, { name: 'thumbnail', maxCount: 1 }]), async (req, res) => {
    try {
...
const createAlbum = [upload.single('thumbnail'), async (req, res) => {
    try {
...
const addMusicToAlbum = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Music added to album successfully', music });
    } catch (error) {
...
const removeMusicFromAlbum = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Music removed from album successfully', music });
    } catch (error) {
...
module.exports = { uploadMusical, createAlbum, addMusicToAlbum, removeMusicFromAlbum, deleteAlbum };
